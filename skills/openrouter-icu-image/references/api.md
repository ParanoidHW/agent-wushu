...
- `POST https://openrouter.icu/v1/files`
...
| 需要复用服务端文件 | 先 `/v1/files` 上传，再 `/v1/images/edits` + JSON `images[].file_id` |
...
| `upload FILE --purpose vision` | multipart `file` + `purpose`，响应中的 `id` / `file_id` 可传给 `edit --file-id` |
...
- 服务端文件：先用 `/v1/files` 上传得到 `file_id`，再用 JSON `images: [{ "file_id": "file_..." }]`。适合复用同一个输入文件或由上游系统预先上传文件。
