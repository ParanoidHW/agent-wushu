...
- `POST https://openrouter.icu/v1/responses`
- `POST https://openrouter.icu/v1/files`（当前服务端可能返回 404，使用前必须重验）
...
- 非图片文档输入（Markdown、TXT、PDF、CSV 等）使用 `/v1/responses` 的 `input_file` + `image_generation` 工具；不要把文档正文塞进 prompt。
- 文档输入默认 Responses 模型使用 `gpt-5.5-medium`。
- 需要多张候选图时使用 `--n`；脚本会在输出数量大于 1 时写成带编号的文件名。
...
| 可公网访问的图片链接 | `/v1/images/edits` + JSON `images[].image_url` |
| Markdown/PDF/TXT 等文档作为生成依据 | `/v1/responses` + `input_file.file_data` + `tools: [{"type":"image_generation"}]` |
| 需要复用服务端图片文件 | 先重验 `/v1/files` 是否可用，再考虑 `/v1/images/edits` + JSON `images[].file_id`；当前 OpenRouter ICU 上 `/v1/files` 曾返回 404 |
...
| `upload` 子命令 | `POST /v1/files`，只在端点重验可用后使用 |
| `responses-doc` 子命令 | `POST /v1/responses`，用 `input_file.file_data` 上传 Markdown/PDF/TXT 等非图片文档 |
| `--prompt` | `prompt` |
| `--model` | `model` |
| `--size` | `size` |
| `--quality` | `quality` |
...
| `upload FILE --purpose vision` | multipart `file` + `purpose`，响应中的 `id` / `file_id` 可传给 `edit --file-id`；当前端点可能不可用 |
| `--output` | 本地保存路径，不发送到 API |
| `--events-output` / `--events_output` | 本地调试日志路径，不发送到 API |
| `--dry-run` | 本地检查请求形状，不发送到 API |

当 `--n` 大于 1 时，`--output output/example.png` 会写成 `output/example-1.png`、`output/example-2.png` 等编号文件。如果服务端返回的最终图片少于请求数量，脚本会报错而不是静默少写。
...
- 本地图片：直接用 multipart `image[]` 上传到 `/v1/images/edits`。这是一次性编辑的首选。
- 图片链接：用 JSON `images: [{ "image_url": "https://..." }]`。链接必须能被 OpenRouter ICU 服务端访问。
- 服务端图片文件：如果 `/v1/files` 在当前会话中已重验可用，可先上传得到 `file_id`，再用 JSON `images: [{ "file_id": "file_..." }]`。当前 OpenRouter ICU 上 `/v1/files` 曾返回 404，不要默认可用。

不要把本地路径、文件 ID 或 URL 写进 `prompt`。这些是请求输入，不是视觉描述。

`/v1/images/edits` 的输入必须是图片 MIME。Markdown 作为 `image[]` 会被拒绝，典型错误是 `Invalid 'input[0].content[1].image_url' ... unsupported MIME type 'text/markdown'`。对非图片文档，改用第 7 节 `/v1/responses`。
...
python3 scripts/openrouter_icu_image.py edit \
  ...
  --prompt "Create a premium product photo using these visual references" \
  --output output/openrouter-icu/references.png
```

### 先上传图片文件再编辑

此路径只用于图片引用复用，并且必须先重验 `/v1/files` 在当前 OpenRouter ICU 服务端可用。不要用它上传 Markdown/PDF/TXT 等文档作为生图依据；文档输入用第 7 节 `/v1/responses`。

```bash
python3 scripts/openrouter_icu_image.py upload input.png --purpose vision
```

上传成功后，脚本会打印 JSON。使用其中的 `file_id`，或 OpenAI 兼容响应里的 `id`：

```bash
python3 scripts/openrouter_icu_image.py edit \
  --file-id file_abc123 \
  --prompt "Change the background while preserving the subject, clothing, pose, and facial features" \
...
```bash
curl -sS \
  -X POST https://openrouter.icu/v1/files \
  -H "Authorization: Bearer $OPENROUTER_ICU_API_KEY" \
  -F "purpose=vision" \
  -F "file=@input.png"
```

如果返回 `404 page not found`，不要继续重试这个路径；改用本地图片 multipart `edit --image`，或对非图片文档改用第 7 节 Responses 文件输入。

## 7. `/v1/responses` 文档输入 + 图像生成

用途：把 Markdown、TXT、PDF、CSV 等非图片文件作为上下文上传，再让模型调用 `image_generation` 工具生成图片。这个路径已验证可用于“将某个 `.md` 作为 upload file，而不只是 prompt”的场景。

请求要点：

- 文件内容放在 `input[].content[]` 的 `input_file` 中。
- `file_data` 使用 data URL：`data:<mime>;base64,<base64 bytes>`。
- 图像生成参数放在 `tools[].type == "image_generation"` 的工具对象里。
- 默认模型使用 `gpt-5.5-medium`；需要覆盖时显式传 `--model`。
- 一次生成多张候选图时，在 `image_generation` 工具对象里设置 `n`，或用 CLI `--n 3`。
- 使用 `stream: true`，避免长图像生成请求被网关 524 截断。
- SSE 最终图片通常在 `response.output[*].result` 或 `response.image_generation_call` 完成项中；中间预览可能在 `partial_image_b64`。
...
payload = {
    ...
    "input": [{
        ...
        "content": [
            {
                "type": "input_text",
                "text": "Create a clean technical infographic from the uploaded document.",
            },
            {
                "type": "input_file",
                "filename": path.name,
...
request = urllib.request.Request(
    ...
    headers={
        "Authorization": "Bearer " + os.environ["OPENROUTER_ICU_API_KEY"],
...
python3 scripts/openrouter_icu_image.py responses-doc \
  ...
  --size 1792x1008 \
  --quality high \
  --output output/openrouter-icu/input-infographic.png
```

该命令会把 `input.md` 作为 `input_file` 上传，并写出 `input-infographic-1.png`、`input-infographic-2.png`、`input-infographic-3.png`。

SSE parsing rules:

- Parse `data:` blocks until `[DONE]` or EOF.
- Abort on events whose `type` contains `failed` or that include `error`.
- Treat `response.image_generation_call.partial_image` / `partial_image_b64` as preview only unless no final image is returned.
- Prefer the final image from `response.completed.response.output[*].result` or `response.output_item.done.item.result`.
- Decode base64 directly; if the payload is a `data:image/...;base64,...` URL, strip the prefix first.

Observed successful image payload locations:

```text
response.image_generation_call.partial_image -> partial_image_b64
response.output_item.done -> item.result
response.completed -> response.output[...].result
```

Operational notes:

- High-resolution `1536x1024` document-input requests may run for several minutes. If they stall, retry `1024x1024`.
- Multi-candidate document requests can be slower and may return fewer images than requested if the provider rejects part of the request. In that case, retry with a smaller `--n` or split into separate requests.
- Keep polling live terminal sessions. `event: keepalive` means the connection is still active.
- `/v1/responses` may revise the image prompt internally; this is normal and can appear in the completed output item as `revised_prompt`.
